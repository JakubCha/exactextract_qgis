...
from qgis.core import QgsCoordinateReferenceSystem, QgsVectorLayer, QgsRasterLayer, QgsGeometry, QgsFeature, QgsField, QgsFields
...
def setup_layers(tmp_path) -> Tuple[QgsVectorLayer, QgsRasterLayer]:
    ...
    for i in range(0, 6):
        feature = QgsFeature()
        feature.setGeometry(QgsGeometry.fromWkt(f"POLYGON((0 {i-1}, 1 {i-1}, 1 {i}, 0 {i}, 0 {i-1}))"))
        feature.setAttributes([i])
        provider.addFeature(feature)
    # Create 5 polygons in the right side of the raster
    for i in range(5, 0, -1):
        feature = QgsFeature()
        feature.setGeometry(QgsGeometry.fromWkt(f"POLYGON((5 {i}, 4 {i}, 4 {i+1}, 5 {i+1}, 5 {i}))"))
    ...
    dataset.SetGeoTransform([0, 1, 0, 5, 0, -1])  # Define the affine transform (how raster is scaled, rotated, skewed, and/or translated)
